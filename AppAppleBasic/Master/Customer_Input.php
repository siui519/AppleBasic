<?php
    ...
    $sql = "SELECT * FROM tblmcust WHERE IdMCust='" . $_GET["dataid"] . "'";
...
<body>
    ...
    <h2><a href="Customer.php">Master Customer</a> / Master Input</h2>

    <div class="container">
    <form <?php if ($_GET["mode"] == "add"){?> action="insert_data.php?master=<?php echo "customer"; ?>&userid=<?php echo $_SESSION['idmuser']; ?>" <?php } else { ?> action="update_data.php?master=<?php echo "customer"; ?>&userid=<?php echo $_SESSION['idmuser']; ?>&dataid=<?php echo $_GET["dataid"]; ?>" <?php } ?> method="post">
        <div class="row">
            <div class="col-25">
                <label for="fname">Kode</label>
            </div>
            <div class="col-75">
            <input type="text" id="fname" name="KdMCust" <?php if ($_GET["mode"] == "add"){ ?> placeholder="Kode.."<?php } else { ?> value = "<?php print($row["KdMCust"]);?>" <?php }?> <?php if ($_GET["mode"] == "view") {?> readonly <?php }?>>
            </div>
        </div>
        <div class="row">
            <div class="col-25">
            ...
            <div class="col-75">
                <input type="text" id="lname" name="NmMCust" <?php if ($_GET["mode"] == "add"){ ?> placeholder="Nama.."<?php } else { ?> value = "<?php print($row["NmMCust"]);?>" <?php }?> <?php if ($_GET["mode"] == "view") {?> readonly <?php }?>>
            </div>
        </div>
        <div class="row">
            <div class="col-25">
        ...
        <div class="row">
            <div class="col-25">
                <label for="lbirthday">Tgl Lahir</label>
            </div>
            <div class="col-75">
                <input type="date" id="lbirthday" name="TglLahir" value = "<?php print($row["TglLahir"]);?>"  <?php if ($_GET["mode"] == "view") {?> readonly <?php }?>>
            </div>
        </div>
        <div class="row">
            <div class="col-25">
